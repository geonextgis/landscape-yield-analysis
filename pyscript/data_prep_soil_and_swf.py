...
parser = argparse.ArgumentParser(
    description="Process SQR and SWF for a given hexagonal grid file."
)
parser.add_argument(
    "--distance",
    type=str,
    required=True,
    help="Distance of the hexagonal grid file (e.g., 2.5, 5, 10)",
)
...
GRID_PATH = os.path.join(
    PROJECT_DATA_DIR, "VECTOR", f"DE_Hexbins_{DISTANCE}sqkm_EPSG_{EPSG}.shp"
)
...
swf_file_path = os.path.join(
    MAIN_DATA_DIR,
    "DE_Small_Woody_Features_5m",
    "HRL_Small_Woody_Features_2018_005m.tif",
)
reprojected_raster_path = os.path.join(
    TEMP_DIR, f"HRL_Small_Woody_Features_2018_005m_{DISTANCE}.tif"
)
...
zones_stats["SWF_area_perc"] = (
    zones_stats["SWF_area_ha"] / zones_stats["area_ha"]
) * 100
zones_stats = zones_stats[["id", "area_ha", "SWF_area_ha", "SWF_area_perc"]]

# Save the data
zones_stats.to_csv(
    os.path.join(out_dir, f"small_woody_featuers_percentage.csv"), index=False
)
